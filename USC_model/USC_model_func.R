model <- function(t, t0, parms) {
  with(as.list(c(t0, parms)), {
    ...
    dS_on <- -lam_on*S_on - community*S_on - testing*(1-p_asympt_stu)*I_on*sensitivity_on*(contacts-R0_on_to_on-R0_student_to_student)*p_contacts_reached + 1/isolation*Q_on*((contacts-R0_on_to_on-R0_student_to_student)/contacts)
    dE_on <-  lam_on*S_on + community*S_on - 1/latent*E_on  - screening_on*E_on*sensitivity_on - testing*(1-p_asympt_stu)*I_on*sensitivity_on*(R0_on_to_on+R0_student_to_student)*p_contacts_reached #last term represents contract tracing
    dI_on <- 1/latent*E_on - testing*(1-p_asympt_stu)*sensitivity_on*I_on - 1/infectious*I_on - screening_on*I_on*sensitivity_on
    dIsym_on <- (1-p_asympt_stu)*(1/latent*E_on - testing*(1-p_asympt_stu)*sensitivity_on*I_on - 1/infectious*I_on - screening_on*I_on*sensitivity_on)
    dR_on <- 1/infectious*I_on + 1/isolation*P_on + 1/isolation*Q_on*((R0_on_to_on+R0_student_to_student)/contacts)
    
    dP_on <- testing*(1-p_asympt_stu)*sensitivity_on*I_on + screening_on*(E_on+I_on)*sensitivity_on - 1/isolation*P_on
    dQ_on <- testing*(1-p_asympt_stu)*I_on*sensitivity_on*contacts*p_contacts_reached  - 1/isolation*Q_on
    ...
    dPcum_on <- testing*(1-p_asympt_stu)*I_on*sensitivity_on + screening_on*(E_on+I_on)*sensitivity_on
    dQcum_on <- testing*(1-p_asympt_stu)*I_on*sensitivity_on*contacts*p_contacts_reached
    ...
    dS_saf <- -lam_saf*S_saf - community*S_saf - testing*(1-p_asympt_saf)*I_saf*sensitivity_saf*(contacts-R0_saf)*p_contacts_reached + 1/isolation*Q_saf*((contacts-R0_saf)/contacts)
    dE_saf <-  lam_saf*S_saf + community*S_saf - 1/latent*E_saf - screening_saf*E_saf*sensitivity_saf - testing*(1-p_asympt_saf)*I_saf*sensitivity_saf*(R0_saf)*p_contacts_reached
    dI_saf <- 1/latent*E_saf - testing*(1-p_asympt_saf)*I_saf*sensitivity_saf- 1/infectious*I_saf - screening_saf*I_saf*sensitivity_saf
    dIsym_saf <- (1-p_asympt_saf)*(1/latent*E_saf - testing*(1-p_asympt_saf)*I_saf*sensitivity_saf- 1/infectious*I_saf - screening_saf*I_saf*sensitivity_saf)
    dR_saf <- 1/infectious*I_saf + 1/isolation*P_saf + 1/isolation*Q_saf*(R0_saf/contacts)
    
    dP_saf <- testing*(1-p_asympt_saf)*I_saf*sensitivity_saf - 1/isolation*P_saf + screening_saf*(E_saf+I_saf)*sensitivity_saf
    dQ_saf <- testing*(1-p_asympt_saf)*I_saf*sensitivity_saf*contacts*p_contacts_reached - 1/isolation*Q_saf
    ...
    dPcum_saf <- testing*(1-p_asympt_saf)*lam_saf*S_saf*sensitivity_saf
    dHcum_saf <- p_hosp_saf*(lam_saf*S_saf + community*S_saf)
    dDcum_saf <- p_death_saf*(lam_saf*S_saf + community*S_saf)

    #Calculated outputs
    dTest <- N_off*screening + N_saf*screening_saf + N_on*screening_on+((I_on+I_off+I_saf)*testing) + N*ili*ifelse(testing > 0, 1, 0) #diagnostics performed
    dCase_stu =   (1-p_asympt_saf)*(1/latent*E_on - testing*(1-p_asympt_stu)*sensitivity_on*I_on - 1/infectious*I_on - screening_on*I_on*sensitivity_on +
